...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
	...
	[headerName setFont:[UIFont fontWithName:@"helvetica Neue Thin" size:11]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(cell == nil) {
		...
		UIImageView *img_Cell = [[UIImageView alloc]initWithFrame:CGRectMake(215, 2, 34, 34)];
		[img_Cell setTag:2];
...
//			[img_Cell setFrame:CGRectMake(214, 2, 35, 34)];
...
//		[invite release];
		...
		if (!self.arr_fbFriendsInfo) {
			...
			if (![eng isLoggedIn]) {
				...
				[eng.facebookAPI.userInfo requestFriendsDetail];
//				NSLog(@"friends may be: %@",eng.facebookAPI.userInfo.friendsInfo);
			}
...
//	[self.dictUserInfo setObject:[userInfo.detailedInfo objectForKey:@"email"] forKey:@"email"];
    ...
    [api setFBIdToken:userInfo.uid txtFBAuthToken:@"1"];
