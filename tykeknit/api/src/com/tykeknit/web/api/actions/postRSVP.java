  ...
  *                                                                           *
  *****************************************************************************
  * $Id: postRSVP.java,v 1.9 2011/06/19 16:27:40 manish Exp $
  *****************************************************************************
  */
...
/**
 * Method:  postRSVP
 *  Posts RSVP for a playdate
 *
 * Required Parameters:
 *  txtPid          - playdate ID
 *  txtRSVP         - RSVP Code (YES, NO, MAYBE)
 *
 * Optional Parameters:
 *  None
 *
...
postRSVP extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, 
      ...
      postRSVPForm postRSVPForm=null;
      ...
      try {
           ...
           postRSVPForm=(postRSVPForm)form;
          ...
          rc = Operations.wallMessagesAuthorization(userTblPk, Integer.parseInt(postRSVPForm.getTxtPid()), ds);
          ...
          if (rc < 0) {
             ...
             jsonStatus.put(messageResources.getMessage("json.response.methodname.displayname"),messageResources.getMessage("json.response.methodname.postrsvp"));
             
             if (rc == -1) {
                 jsonResponse.put(messageResources.getMessage("json.response.reasoncode.displayname"),messageResources.getMessage("json.response.methodname.postrsvp.message.failure.reasoncode.1"));
                 jsonResponse.put(messageResources.getMessage("json.response.reasonstr.displayname"),messageResources.getMessage("json.response.methodname.postrsvp.message.failure.reasonstr.1"));
             }
             else if (rc == -2) {
          ...
          rc = Operations.postRSVP(userTblPk, postRSVPForm,ds);  
          ...
          if (rc < 0) {
              ...
              sqlString += "from user_tbl            a "
                         + "    ,playdate_tbl        b ";
              sqlString += "where b.playdate_tbl_pk = " + postRSVPForm.getTxtPid() + " "
                         +   "and a.user_tbl_pk =  b.playdate_organiser_user_tbl_fk ";
              ...
              subject = messageResources.getMessage("emailmessage.postRSVP.subject");
              subject = subject.replace("<FName>", user.getUserFName());
              subject = subject.replace("<LName>", user.getUserLName());
              
              if ("YES".equalsIgnoreCase(postRSVPForm.getTxtRSVP())) {
                  message = messageResources.getMessage("emailmessage.postRSVP.body.yes");
              }
              else if ("NO".equalsIgnoreCase(postRSVPForm.getTxtRSVP())) {
                  message = messageResources.getMessage("emailmessage.postRSVP.body.no");
              }
              else {
                  message = messageResources.getMessage("emailmessage.postRSVP.body.maybe");
              }
              ...
              message = message.replace("<PlaydateName>", rs.getString(2));
