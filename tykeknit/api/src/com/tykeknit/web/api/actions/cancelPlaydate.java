  ...
  *                                                                           *
  *****************************************************************************
  * $Id: cancelPlaydate.java,v 1.5 2011/05/13 01:59:21 manish Exp $
  *****************************************************************************
  */
package com.tykeknit.web.api.actions;

import com.tykeknit.web.api.actionforms.cancelPlaydateForm;
 ...
 *
 *  See Also:
 *      <getPlaydateParticipants>, <playdateDetails>, <playdateRequest>
 *
 */

public class

cancelPlaydate extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, 
      ...
      cancelPlaydateForm cancelPlaydateForm=null;
      ...
      try {
           ...
           cancelPlaydateForm=(cancelPlaydateForm)form;
          ...
          int playdate_id = Integer.parseInt(cancelPlaydateForm.getTxtPid());
          
          rc = Operations.cancelPlaydate(userTblPk,playdate_id,ds);  
          ...
          if (rc < 0) {
              ...
              jsonStatus.put(messageResources.getMessage("json.response.methodname.displayname"),messageResources.getMessage("json.response.methodname.cancelplaydate"));
              
              if (rc == -1) {
                  jsonResponse.put(messageResources.getMessage("json.response.reasoncode.displayname"),messageResources.getMessage("json.response.methodname.cancelplaydate.message.failure.reasoncode.2"));
                  jsonResponse.put(messageResources.getMessage("json.response.reasonstr.displayname"),messageResources.getMessage("json.response.methodname.cancelplaydate.message.failure.reasonstr.2"));
              }
              else if (rc == -2) {
              ...
              subject = messageResources.getMessage("emailmessage.cancelPlaydate.subject");
              subject = subject.replace("<FName>", user.getUserFName());
              subject = subject.replace("<LName>", user.getUserLName());

              message = messageResources.getMessage("emailmessage.cancelPlaydate.body");
              ...
              sqlString += "where a.playdate_tbl_pk = " + cancelPlaydateForm.getTxtPid() + " ";
              ...
              String playdateName = rs.getString(1);
              String playdateLocation = rs.getString(2);
              String playdateStartdate = rs.getString(3);

              subject = subject.replace("<PlayedateName>", playdateName);
              message = message.replace("<PlayedateName", playdateName);
              ...
              sqlString += "where b.playdate_tbl_fk = " + cancelPlaydateForm.getTxtPid() + " "
                         +   "and b.parent_user_tbl_fk = a.user_tbl_pk "
              ...
              while (rs.next()) {
                  rc = Operations.sendDBMessage(userTblPk, Integer.parseInt(rs.getString(3)), subject, message, ds);
